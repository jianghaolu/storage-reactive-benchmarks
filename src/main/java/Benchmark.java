...
public abstract class Benchmark {
    ...
    public static final String ACCOUNT_NAME = System.getenv("ACCOUNT_NAME");
    // blob container name
    public static final String CONTAINER_NAME = System.getenv("CONTAINER_NAME");
    // SAS token, starting with "?"
    public static final String SAS_TOKEN = System.getenv("SAS_TOKEN");
    // The folder where large files can be downloaded and uploaded. For VHD benchmarks, this should have at least 130G free space
    public static final String FILE_ROOT = System.getenv("FILE_ROOT");

    public static final String URL = String.format("https://%s.blob.core.windows.net/%s/", ACCOUNT_NAME, CONTAINER_NAME);
    public abstract FileClient client(String url, String sasToken);
...
//        upload20One1GB();
    ...
    public void uploadVhd() throws Exception {
        ...
        client(URL, SAS_TOKEN).uploadPageBlobAsync("test.vhd", "D:\\out.vhd", fileSize, blockSize)
                .reduce((a, b) -> {
                    long downloaded = a + b;
                    trackProgress(downloaded, fileSize, b, tracker);
                    return downloaded;
